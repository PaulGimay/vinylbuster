      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
              ...
              <div class="row"><div class="fw-bold mt-3 fontContent">Description</div></div>
              <div class="row"><div data-hifioverview class="fontContent overviewHeigth overflow-auto hidescroll"></div></div>
...
let Hifioverview = document.querySelectorAll('[data-hifioverview')
...
item.equip.forEach((item,i)=>{
    ...
    Hifioverview[i].innerHTML = item.overview
})
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
              ...
              <div class="row"><div class="fw-bold mt-3 fontContent">Description</div></div>
              <div class="row"><div class="fontContent"></div></div>
        ...
        element.addEventListener('click', function(){
            ...
            Cart.forEach((element,i) => {
                ...
                <div data-cart-item="${element[0]}" class="row mb-2">
                    ...
                    <div class="col-9 col-lg-4 fs-5" data-cart-name="${element[1]}">${element[1]} - #${element[0]}</div>
                    <div class="col-lg-5">
                        <div class="row">
                            <div class="col-3 fs-5 text-center" data-cart-price="${element[2]}">${element[2]}€</div>
                            <div class="col-3 fs-5 text-center" data-cart-qt-id="${element[0]}" data-cart-qt="${element[3]}">${element[3]}</div>
                            <div class="col-4 text-center"><button data-plus="${element[0]}" class="btnReset">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg></button><button data-minus="${element[0]}" class="btnReset">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-minus"><line x1="5" y1="12" x2="19" y2="12"></line></svg></button></div>
                            <div class="col-2 text-center"><button data-trash="${element[0]}" class="btnReset">
            ...
            /************************ BOUTON PLUS PANIER ***************************/
                ...
                btnPlus.addEventListener('click', function(){
                    if(element[3]++ < 10){  // vérif si après augmentation toujours < à 10
                        // console.log(element[3])
                        element[3]          // si oui on augmente
                        total = Math.round( (total + parseFloat(element[2])) * 100) / 100       // on met à jour le total virtuellement
                        // console.log(element[3])
                        divQt.dataset.cartQt = element[3]       // on change la quantité virtuellement
                        divQt.innerHTML = element[3]            // on change la quantité visuellement 
                        // console.log(divQt.dataset.cartQt)
                        // console.table(element)
                        console.table(Cart)
                        console.log(total)
                        innerTotal.innerHTML = 'Total : ' + total + ' €'        // on met à jour le total visuellement

                    } else {
                        alert('Vous ne pouvez achetez plus de 10 fois le même article')     // si non, impossible d'augmenter la qt
                    }
                })
            /************************ BOUTON MOINS PANIER ***************************/

                btnMinus.addEventListener('click', function(){
                    if(element[3]-- > 1){           // vérif si après diminution toujours > à 1
                        // console.log(element[3])
                        element[3]                  // si oui on diminue
                        // console.log(element[3])
                        divQt.dataset.cartQt = element[3]           // on change la quantité virtuellement
                        divQt.innerHTML = element[3]                // on change la quantité visuellement 
                        // console.log(divQt.dataset.cartQt)
                    ...
                    } else {        //Si non, on retire la ligne ET on supprime l'item virtuellement dans le panier, et visuellement
                        ...
                        idParentOfMinus.remove()        // on remove l'html
                        // console.table(Cart)
                        Cart.splice(i,1)        // on remove l'element dans le tableau
                        total = Math.round( (total - parseFloat(element[2])) * 100) / 100       // màj du total virtuellement
                        console.table(Cart)
                        console.log(total)
                        innerTotal.innerHTML = 'Total : ' + total + ' €'        // màj du total visuellement
                    }
                })
            })
            ...
            TrashButtons.forEach(element => {
              element.addEventListener('click', function(){
                ...
                idParentOfTrash.remove()            // remove la ligne de l'html

                Cart.forEach((element,i)=>{             // on parcours le tableau panier pour trouver l'item sur lequel on a clické
                    if(element[0] == idParentOfTrash.dataset.cartItem){
                        total = Math.round((total - parseFloat(element[3]) * parseFloat(element[2])) * 100) / 100
                        Cart.splice(i,1)                // puis on le remove du tableau
